...
# Check if we should use a dummy cache (for development without Redis)
USE_DUMMY_CACHE = config("USE_DUMMY_CACHE", default=False, cast=bool)
NO_REDIS = config("NO_REDIS", default=False, cast=bool)
...
if USE_DUMMY_CACHE:
    ...
    # Use Redis cache (production/normal development)
    CACHES = {
        "default": {
...
# If NO_REDIS is set, run tasks synchronously
if config("NO_REDIS", default=False, cast=bool):
